...
namespace DropoutCoder.SmartGuideTdd.FirstNonConsecutiveNumber.Tests
{
    [TestFixture]
    public class LinqBrokenSequenceFinderTests
    {
        [Test]
        public void Array_Is_Null_Throws_ArgumentNullException()
        {
            var finder = new LinqBrokenSequenceFinder();
        ...
        [TestCase(new[] { int.MinValue, int.MinValue + 1, int.MinValue + 2 }, null)]
        [TestCase(new[] { int.MaxValue - 2, int.MaxValue - 1, int.MaxValue }, null)]
        [TestCase(new[] { int.MinValue, int.MinValue + 3, int.MinValue + 5 }, int.MinValue + 3)]
        [TestCase(new[] { int.MinValue, 0, 1 }, 0)]
        [TestCase(new[] { 0, 1, int.MaxValue - 3, int.MaxValue }, int.MaxValue - 3)]
